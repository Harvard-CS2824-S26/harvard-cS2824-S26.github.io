...
This assumtion is sutle. It
does permit polynomial sample complexity both with a generative model (via
adaptive, on-policy rollouts) and in an episodic setting (see the discussion in
Section~\ref{chapterBC_bib}). However, as we see in this chapter, (C)
it still does \emph{not} guarantee sample-efficient \emph{offline} policy
evaluation: even with extremely strong feature coverage, any estimator can require
exponentially many samples (Section~\ref{sec:linear_Qvalues}).
...
and avoiding the worst-case compounding that occurs with fixed data (see
\citet{du2019good}).
Furthermore, polynomial-sample PAC guarantees under~(C) are also available in the
online episodic setting. Here,
\citet{weisz2023online_qpi} shows that linearly $Q^\pi$-realizable MDPs
are PAC learnable with sample complexity polynomial in
$(d,H,1/\epsilon)$.  Related polynomial-sample guarantees for online
learning under $Q^\pi$-realizability (with computational efficiency
and with more restrictions) are obtained by
\citet{mhammedi2024optimistic_psdp}.  This makes the (C) model subtle:
naive least-squares dynamic programming can fail offline, yet the model is still learnable.
