...
estimated from the last layer, causing an exponential dependence on the horizon.
...
while action $a_1$ (solid) jumps to the aggregator chain $s_h^i\to g_{h+1}$.
From aggregator states, both actions transition to the next aggregator state (shown by the two parallel edges).
}
