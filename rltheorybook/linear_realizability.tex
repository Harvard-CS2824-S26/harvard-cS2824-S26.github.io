\chapter{RL with Linear Realizability}
...
In this chapter, we focus on doing RL where we have access to some
feature mapping $\phi:\Scal\times\Acal\mapsto
\mathbb{R}^{d}$. Here we hope that class of linear functions $\{
f(s,a) := \theta^{\top} \phi(s,a)| \theta \in \R^d$ has some good
approximation properties to to the value function, in some precise
sense.  We seek to understand under what conditions we can do sample
efficient reinforcement learning, i.e. only $poly(d)$ dependencies.

For this chapter we will work in the finite horizon setting.
SK: maybe say why? in some ways, it is more intuitive to think in
terms of the backward induction view of dynamic programming.


SK: I need help organizing the below. The chapter outline will be:

something about agnostic learning and then diff notions of linear
realiziability

then we will just do regression detour

then we will do LSPE/LSVI with BC.
- offline RL will be corollary

then we will do lower bounds

%\section{Agnostic RL with Linear Features}

Note that we can view our features as a matrix of size $|\Scal|\cdot
|\Acal| \times d$, which we will denote by $\Phi$.

The most ambitious objective would be fine some $w$ which solves the
following opitmization problem:
\[
\min_{w\in\R^d}\|Q^\star - \Phi w\|_{\star}
\]
where $\|\cdot \|_\star$ is either some norm like the $\ell_\infty$
norm or distributionally weighted error measure.

For the case of supervised learning, the analgous problem is that of:
\[
\min_{w\in\R^d} \EE \min_{(x,y)\sim D} [(y-w\cdot \psi(x))^2].
\]
We would say the problem is realizable if $E[y|x] = w^\star \cdot
\psi(x)$. However, even when the the problem is not realizable. 

\section{Notions of Linear Realizability}
