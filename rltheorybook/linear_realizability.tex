...
issues---to the chapter on fitted value iteration.
However, the lower bounds presented later in this chapter for stronger
assumptions (like realizability) imply that the agnostic viewpoint is
prone to exponential sample complexity without further
...
Finally, note a key subtlety about data access: while (C) is still insufficient
for sample-efficient \emph{offline} RL (Section~\ref{sec:linear_Qvalues}), it
\emph{does} permit polynomial sample complexity with a generative model via
adaptive, on-policy rollouts; see the discussion in
Section~\ref{chapterBC_bib}.
...
\paragraph{A remark on feature augmentation (non-monotonicity).}
A perhaps surprising subtlety is that Bellman completeness is \emph{not} monotone in the
feature map.
If we enlarge the representation by appending an additional coordinate to $\phi$
(thus enlarging $\Fcal$ from a $d$-dimensional linear class to a $(d+1)$-dimensional one),
the closure property $f\in\Fcal \Rightarrow \Tcal_h f\in\Fcal$ can \emph{break}, even if it held before.
Intuitively, completeness must hold for \emph{every} linear function in the class; adding
a new feature introduces new functions $f$ whose Bellman backups may no longer be
representable by the enlarged linear class.
In contrast, realizability assumptions such as linear $Q^\star$ (or all-policies
realizability) are monotone: adding features can only make it easier to represent the
target functions. This fragility is another reason to view Bellman completeness as a
structural assumption about the interaction between the MDP and the chosen feature map,
rather than a mere approximation property of $Q^\star$.
...
(Section~\ref{sec:assumption_ladder}).
...
\[
D_h = \{(s_i,a_i,r_i,s_i')\}_{i=1}^{n_h}
\]
be a dataset of $n_h$ transitions at stage $h$, where each
...
unique minimum-volume centered ellipsoid containing $\Phi$; this
result is known as John's Theorem.  We do not provide a
...
Assume Bellman completeness and bounded rewards
...
The first step looks at one stage as fixed-design regression.
...
By Bellman completeness, there exists $\tilde\theta_h\in\R^d$ such that for all
$(s,a)$,
\[
\tilde\theta_h^\top \phi(s,a)
=
r_h(s,a) + \E_{s'\sim P_h(\cdot\mid s,a)}\big[\widehat V_{h+1}(s')\big]
=
(\Tcal_h \widehat Q_{h+1})(s,a).
\]
Hence $y_i = \tilde\theta_h^\top x_i + \xi_i$ where $\E[\xi_i\mid x_i]=0$ and
$\{\xi_i\}$ are independent across samples. Moreover $|\xi_i|\lesssim H$, so
$\xi_i$ is $O(H)$-sub-Gaussian.

Let $n_h:=|D_h|$. For notational consistency with the Appendix, we normalize all
second-moment matrices by $N$ (the nominal per-stage sample size used in the construction)
and define
\[
\Lambda_h
\;:=\;
...
\]
(Equivalently, if we also define the usual empirical covariance
$\widehat\Sigma_h := \frac{1}{n_h}\sum_{(s,a,r,s')\in D_h}\phi(s,a)\phi(s,a)^\top$,
then $\Lambda_h = \frac{n_h}{N}\,\widehat\Sigma_h$.)

\paragraph{Step 2: leverage control from D-optimal sampling (normalized).}
Let $\Sigma=\E_{(s,a)\sim\rho}[\phi(s,a)\phi(s,a)^\top]$. By construction of $D_h$,
\[
...
N\Sigma.
\]
Dividing by $N$ yields $\Lambda_h\succeq \Sigma$, hence $\Lambda_h^{-1}\preceq \Sigma^{-1}$ and,
by Lemma~\ref{lem:d_opt_design},
\[
\sup_{(s,a)} \phi(s,a)^\top \Lambda_h^{-1}\phi(s,a)
\;\le\;
\sup_{x\in\Phi} x^\top\Sigma^{-1}x
\;\le\; d.
\]

\paragraph{Step 3: OLS generalizes uniformly (normalized).}
Apply the fixed-design OLS bound from the Appendix (Theorem~\ref{thm:ols_fixed_design})
to the stage-$h$ regression with sample size $n_h$, failure probability
$\delta' := \delta/H$, and noise scale $\sigma=O(H)$. Using the empirical covariance
$\widehat\Sigma_h$ (normalized by $n_h$), we obtain with probability at least $1-\delta'$,
\[
\|\widehat\theta_h-\tilde\theta_h\|_{\widehat\Sigma_h}
\;\le\;
c_1 H \cdot \frac{\sqrt{d}+\sqrt{2\log(1/\delta')}}{\sqrt{n_h}}
\;=\;
c_1 H \cdot \frac{\sqrt{d}+\sqrt{2\log(H/\delta)}}{\sqrt{n_h}},
\]
for an absolute constant $c_1$.
Since $\Lambda_h = \frac{n_h}{N}\widehat\Sigma_h$, we have
\[
\|\widehat\theta_h-\tilde\theta_h\|_{\Lambda_h}
=
\sqrt{\frac{n_h}{N}}\;\|\widehat\theta_h-\tilde\theta_h\|_{\widehat\Sigma_h}
\;\le\;
c_1 H \cdot \frac{\sqrt{d}+\sqrt{2\log(H/\delta)}}{\sqrt{N}}.
\]
Therefore, for any $(s,a)$,
\begin{align*}
\big|(\widehat\theta_h-\tilde\theta_h)^\top \phi(s,a)\big|
&\le
\|\widehat\theta_h-\tilde\theta_h\|_{\Lambda_h}\cdot
...
\;\le\;
c_1 H \cdot \frac{\sqrt{d}\big(\sqrt{d}+\sqrt{2\log(H/\delta)}\big)}{\sqrt{N}}.
\]

Finally, apply the bound for all $h\in[H]$ and take a union bound: with probability at least
$1-\delta$,
\[
...
completeness and coverage, the proof of the
...
Assume Bellman completeness, bounded rewards
...
Concretely, at stage $h$, define the normalized covariance
\[
\Lambda_h
\;:=\;
\frac{1}{N}\sum_{(s,a,r,s')\in D_h}\phi(s,a)\phi(s,a)^\top
...
\widehat\Sigma_h.
\]
Then Assumption~\ref{assumption:coverage} implies $\Lambda_h\succeq \Sigma/\kappa$, hence
\[
\sup_{(s,a)}\phi(s,a)^\top\Lambda_h^{-1}\phi(s,a)
\;\le\;
\kappa\cdot \sup_{(s,a)}\phi(s,a)^\top\Sigma^{-1}\phi(s,a)
...
\[
r_h(s,a)+\E[\widehat V_{h+1}^\pi(s')\mid s,a]=(\tilde\theta_h)^\top\phi(s,a)
\]
for some $\tilde\theta_h\in\R^d$. Thus each stage is a fixed-design linear regression with
$O(H)$-sub-Gaussian noise. Define the normalized covariance
$\Lambda_h:=\frac{1}{N}\sum_{(s,a,r,s')\in D_h}\phi(s,a)\phi(s,a)^\top=\widehat\Sigma_h$.
Coverage implies $\Lambda_h\succeq \Sigma/\kappa$, hence
\[
\sup_{(s,a)}\phi(s,a)^\top \Lambda_h^{-1}\phi(s,a)
\;\le\;
\kappa d.
...
In Appendix~\ref{app:linearQstar_lb}, we state and prove a weaker
version of this theorem based on a simpler construction with
$|\Acal|=\exp(d)$ (though, even for this weaker statement, the construction is
quite involved).
...
Note that this theorem says it not possible to estimate the value of
\emph{any} given policy, from the starting state $s_0$.
...
On aggregator states, $r_h(g_h,\cdot)=\alpha_h-\alpha_{h+1}$ for $h\le H-2$ and $r_{H-1}(g_{H-1},\cdot)=\alpha_{H-1}=r_\infty\sqrt m$.
(Note that $\alpha_h:=r_\infty m^{(H-h)/2}$).
...
We now verify Assumption~\ref{assmp:realizability_all_policies}.
...
With regard to the learnability of the all-policies linear
realizability model~(C) in online (episodic) settings,
\citet{weisz2023online_qpi} show that linearly $Q^\pi$-realizable MDPs
are PAC learnable with sample complexity polynomial in
$(d,H,1/\epsilon)$ (despite the offline impossibility results
discussed above).  Related polynomial-sample guarantees for online
learning under $Q^\pi$-realizability (with computational efficiency
and with more restrictions) are obtained by
\citet{mhammedi2024optimistic_psdp}. 

\paragraph{A note on (C) with a generative model.}
Our exponential lower bound under all-policies linear realizability (C) is an
\emph{offline} phenomenon: the fixed dataset can systematically miss the
state--action pairs whose values drive $V^\pi$, forcing repeated extrapolation
through Bellman backups and allowing estimation error to amplify across stages.
With a generative model, this particular obstruction disappears because the
learner can collect \emph{fresh} samples under (or targeted to) the current
candidate policy, producing unbiased estimates of the relevant Bellman targets
and avoiding the worst-case compounding that occurs with fixed data.
This makes the (C) model genuinely subtle: naive least-squares dynamic
programming can fail offline, yet the model is still PAC-learnable with a
generative model by procedures that leverage adaptive rollouts; see
\citet{du2019good}.
Polynomial-sample PAC guarantees under~(C) are also available in the
online episodic setting, with more involved algorithms (see Section~\ref{chapterBC_bib} for
discussion).
...
under the true optimal continuation rather than a biased surrogate—precisely the
