...
\begin{align*}
    \textrm{Regret} := \EE \left[ \sum_{k=0}^{K-1} \left(V^{\star}  - V^{\pi^k} \right)  \right],
...
Q^\star_{h}(s,a) & = \theta_h^\star\cdot \phi(s,a) + \EE_{s\sim P_h(\cdot | s,a)} V_{h+1}^\star(s') = \theta_h^\star \cdot \phi(s,a) + P_h(\cdot | s,a) \cdot V^\star_{h+1} = \theta_h^\star \cdot \phi(s,a) +  (\mu_h^\star \phi(s,a))^{\top} V^\star_{h+1}  \\
...
\begin{align*}
\EE\left[ \epsilon_{h}^i \vert \Hcal_{h}^i \right] = 0,
...
\begin{align*}
\EE\left[ V^{\top} \epsilon_h^i  \vert \Hcal_{h}^i \right]  = 0, \quad | V^{\top} \epsilon_h^i  | \leq \|V\|_{\infty} \|\epsilon_h^i \|_1 \leq 2 H, \forall h,i.
\end{align*}
...
\begin{align*}
 \EE\left[ N V^\star - \sum_{n=1}^N V^{\pi_n} \right] & = \EE\left[  \one\{\Ecal_{model} \text{ holds} \} \left(N V^\star - \sum_{n=1}^N V^{\pi_n} \right)\right] \\
 & \qquad + \EE\left[  \one\{\Ecal_{model} \text{ doesn't hold} \} \left(N V^\star - \sum_{n=1}^N V^{\pi_n} \right)\right] \\
& \leq   \EE\left[  \one\{\Ecal_{model} \text{ holds} \} \left(N V^\star - \sum_{n=1}^N V^{\pi_n} \right)\right] + \delta NH \\
& \lesssim \EE  \left[    \sum_{n=1}^N \sum_{h=0}^{H-1} b_h^n(s_h^n,a_h^n)    \right] + \delta NH.
\end{align*}
Note that:
\begin{align*}
