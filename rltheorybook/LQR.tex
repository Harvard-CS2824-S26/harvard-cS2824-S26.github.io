...
+ u^\top R u
\end{align*}
due to that $x_H = A_{H-1}x+B_{H-1}u+w_{H-1}$, and $\EE[ w_{H-1}^{\top} Q w_{H-1} ] = \textrm{Trace}(\sigma^2 Q)$.
...
This implies that:
\begin{align*}
Q^\star_{H-2}(x,u) &=  \EE[V^\star_{H-1}(A_{H-2}x+B_{H-2}u+w_{H-2})]
+x^\top Q x+u^\top R u\\
...
\begin{align*}
\EE_{\pi} \left[ \sum_{t=0}^{H-1} c_t(x_t, u_t)   \right],
