...
The first part of the proof is with regards to model accuracy.
...
Now we characterize the uniform value accuracy.
...
We bound the near-optimal planning accuracy.
...
\paragraph{Lower bounds.}
In the same minimax sense as Theorem~\ref{thm:minimax_lower_discounted}, the above model-based rate is optimal (up to constant and logarithmic factors) for finite-horizon MDPs under a generative model. We omit the proof.


\section{Analysis}\label{sec:analysis}

We now prove the \emph{value-estimation} guarantee in Theorem~\ref{thm:sample_complexity_finite}
(i.e.\ the first bullet). For convenience, we restate it here.

\begin{theorem}[Value estimation; restatement of Theorem~\ref{thm:sample_complexity_finite}]
...
\begin{proof}[Proof of Theorem~\ref{thm:sample_complexity_value_est}]
...
\Big\|(I-\gamma \widehat P^{\pi^\star})^{-1}\sqrt{\Var_{\widehat P}(\widehat V^{\pi^\star})}\Big\|_\infty
  \\
  &\quad \quad \;+\;\frac{c\gamma}{(1-\gamma)^2}\left(\frac{\log(c|\Scal||\Acal|/\delta)}{N}\right)^{3/4}
...
\section{Bibliographic Remarks and Further Reading}\label{bib:sample_complexity}

The generative model framework (also known as a simulator oracle) was introduced by \citet{kearns1999finite}, who established that model-based and model-free methods have comparable sample complexity up to horizon and logarithmic factors. \citet{kakade2003sample} provided improved rates analogous to the ``crude'' bounds presented in Proposition~\ref{proposition:crude}.

The sharp value estimation bound (the first claim in
Theorem~\ref{thm:minimax_upper_discounted}) is due to
\citet{azar2013minimax}; the analysis in Section~\ref{sec:analysis}
largely follows their technique. Regarding the sub-optimality of the
learned policy $\widehat \pi^\star$, a naive analysis suggests an
amplification of the error by
$(1-\gamma)^{-1}$. \citet{sidford2018vnear} provided the first proof
that this amplification can be avoided, achieving the optimal rate
using a variance-reduced value iteration algorithm. Subsequently,
\citet{pmlr-v125-agarwal20b} showed that the naive model-based
approach (the ``plug-in'' estimator) is also sufficient to achieve
this optimal rate (the second claim in
Theorem~\ref{thm:minimax_upper_discounted}). The minimax lower bound
(Theorem~\ref{thm:minimax_lower_discounted}) is due to
\citet{azar2013minimax}. 

We also note that the sub-optimality bounds presented here require $\eps$ to be sufficiently small (roughly $\eps \lesssim 1/\sqrt{1-\gamma}$). \citet{li2020breaking} showed that the optimal rate is achievable for the full range of meaningful accuracy (up to $\eps \approx 1/(1-\gamma)$) using a perturbed model-based algorithm. It remains an open question whether the standard (unperturbed) model-based approach achieves this optimal non-asymptotic behavior for large $\eps$.

Finally, this chapter presented optimal sample complexity results for
the finite-horizon setting (Section~\ref{sec:finite_horizon_sc})
without proof. These results follow from the line of reasoning in
\citet{azar2013minimax}, with the added
simplification that the sub-optimality analysis is simpler in the
finite-horizon setting with time-dependent transition matrices (see,
e.g., \cite{Yin2021NearOP}).
