...
is the Euclidean norm of a $d$-dimensional sub-Gaussian vector (e.g. see also \citep{HsuSpectral}).
...
\[
\Lambda := \sum_{i=1}^N x_ix_i^\top
\]
and assume $\Lambda$ is invertible. The ordinary least squares estimator is
\[
\hat\theta := \Lambda^{-1}\sum_{i=1}^N x_i y_i.
\]
Then for any $\delta\in(0,1)$, with probability at least $1-\delta$,
\[
\|\hat\theta-\theta^\star\|_{\Lambda}
\le
\sigma\Big(\sqrt{d}+\sqrt{2\log(1/\delta)}\Big),
\]
and hence
\[
\|\hat\theta-\theta^\star\|_{\Lambda}^2
\le
\sigma^2\Big(d + 2\sqrt{2d\log(1/\delta)} + 2\log(1/\delta)\Big).
\]
\end{theorem}
