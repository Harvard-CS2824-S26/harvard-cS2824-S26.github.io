...
\]
The learner must output a policy $\widehat{\pi}$ using only $\Dcal$.
...
The following theorem bounds the performance loss by three terms:
(i) a statistical term scaling as $n^{-1/2}$,
(ii) an approximation term governed by $\epsilon_{\mathrm{approx},\nu}$, and
(iii) a finite-iteration term that decays geometrically in $K$.
...
\frac{2}{(1-\gamma)^2}
\left(
\sqrt{\frac{20\,C\,\vmax^2\,\ln\!\big(2|\Fcal|^2 K/\delta\big)}{n}}
\;+\;
\sqrt{3\,C\,\epsilon_{\mathrm{approx},\nu}}
\right)
\;+\;
...
The dependence on $C$ is the explicit coverage price: regression error under $\nu$
must transfer to the visitation distributions induced by the greedy policies.
The term $\epsilon_{\mathrm{approx},\nu}$ is the Bellman approximation price:
even with infinite data, lack of (average-case) Bellman closure creates an error floor.
The final term is purely algorithmic, from running only $K$ fitted iterations.

\subsection*{Proofs for Theorem~\ref{thm:fqi_main_ch4}}

We combine a propagation bound (Bellman residuals $\Rightarrow$ performance loss)
with a uniform regression bound for each fitted step.

\begin{lemma}[Bellman residuals $\Rightarrow$ performance loss]\label{lem:approximate_be_fqi}
Assume that for all $t=0,1,\dots,K-1$ we have
\[
\|f_{t+1}-\Tcal f_t\|_{2,\nu}\le \varepsilon.
\]
Let $\pi^t$ be greedy w.r.t.\ $f_t$. Then for every $k\in\{0,1,\dots,K\}$,
\[
V^\star - V^{\pi^k}
\;\le\;
...
\begin{proof}
A standard approximate value-iteration bound (see, e.g., the fitted value-iteration
analyses in \cite{munos2005error,antos2008learning}) implies that for greedy policies
$\{\pi^t\}$,
\[
V^\star - V^{\pi^k}
\;\le\;
\frac{2}{1-\gamma}\sum_{t=0}^{k-1}\gamma^{k-1-t}\,
\|f_{t+1}-\Tcal f_t\|_{1,d^{\pi^t}_\mu}
\;+\;
\frac{2\gamma^k\,\vmax}{1-\gamma}.
\]
By Assumption~\ref{assum:concentrability_fqi} and Cauchy--Schwarz,
\[
\|f_{t+1}-\Tcal f_t\|_{1,d^{\pi^t}_\mu}
\le
\|f_{t+1}-\Tcal f_t\|_{2,d^{\pi^t}_\mu}
\le
\sqrt{C}\,\|f_{t+1}-\Tcal f_t\|_{2,\nu}
\le
\sqrt{C}\,\varepsilon.
\]
Using $\sum_{t=0}^{k-1}\gamma^{k-1-t}\le (1-\gamma)^{-1}$ yields the claim.
\end{proof}
...
\| f_{t+1} - \Tcal f_t \|_{2,\nu}^2
\;\le\;
\frac{20 \vmax^2 \ln\!\big(2|\Fcal|^2 K/\delta\big)}{n}
\;+\;
...
\]
Then $\E[y_i\mid s_i,a_i]=(\Tcal g)(s_i,a_i)$. Since $r\in[0,1]$ and $g\in[0,\vmax]$,
\[
0\le y_i \le 1+\gamma\vmax = \vmax,
\qquad
0\le (\Tcal g)(s,a)\le \vmax.
\]
By Assumption~\ref{assum:ibe}, $\min_{f\in\Fcal}\|f-\Tcal g\|_{2,\nu}^2\le \epsilon_{\mathrm{approx},\nu}$.
Applying Lemma~\ref{lem:least_square_gen} with $B=\vmax$ gives that, with probability at least
$1-\delta$, for this fixed $g$,
\[
\|\widehat f_g - \Tcal g\|_{2,\nu}^2
\le
\frac{20 \vmax^2 \ln\!\big(2|\Fcal|/\delta\big)}{n}
+ 3\,\epsilon_{\mathrm{approx},\nu},
\]
where $\widehat f_g$ is the ERM over $\Fcal$ for targets $y_i$.
A union bound over $g\in\Fcal$ and $t=0,\dots,K-1$ (equivalently, replace $\delta$ by
$\delta/(|\Fcal|K)$) yields the stated bound, and $f_{t+1}=\widehat f_{f_t}$ by construction.
\end{proof}
...
\| f_{t+1} - \Tcal f_t \|_{2,\nu}
\le
\sqrt{\frac{20 \vmax^2 \ln\!\big(2|\Fcal|^2 K/\delta\big)}{n}}
+
\sqrt{3\,\epsilon_{\mathrm{approx},\nu}}.
\]
Set
\[
\varepsilon :=
...
\sqrt{3\,\epsilon_{\mathrm{approx},\nu}}.
\]
Applying Lemma~\ref{lem:approximate_be_fqi} with this $\varepsilon$ and $k=K$ yields
\[
V^\star - V^{\pi^K}
\le
...
$\Dcal$ to a policy $\widehat\pi$, we have
\[
\max\Big\{\E_{\Dcal\sim \mathcal{M}_+}\big[V^\star - V^{\widehat\pi}\big],\;
         \E_{\Dcal\sim \mathcal{M}_-}\big[V^\star - V^{\widehat\pi}\big]\Big\}
\;\ge\; \frac{1}{4}\vmax.
\]
\end{proposition}
...
algorithm $\Acal$ induces the same distribution over outputs $\widehat\pi$ under both
MDPs.
