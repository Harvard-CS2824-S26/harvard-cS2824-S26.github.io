...
Chapter~\ref{chap:linear_features} which suggest that
...
V_h(s) -\max_a\left( r_h(s,a) + \E_{s' \sim P_h(\cdot|s, a)}\left[ V_{h+1} (s') \right] \right) =0.
\end{equation}
...
%\begin{align*}
%\E_{s\sim \mu} \left[  V_{0,g}(s) - r(s, \pi_{0,g}(s))   \right] =  \sum_{a\in\Acal}  \E_{s\sim \mu} \left[ \one\{ \pi_g(s) = a \} \left[ Q_{0,g}(s,a) - r(s,a) \right]\right]. 
%\end{align*} This means that we can write the Bellman error of $g$ averaged over $\mu$ as an inner product between two vectors --- a all one vector, and a vector whose $a$-th entry is $\E_{s\sim \mu}\left[  \one\{ \pi_g(s) = a \} \left[ Q_{0,g}(s,a) - r(s,a) \right]\right]$.
...
\end{align*}  Namely, we can set $W_0(g) = w$, and $X_0(f) = \E_{s\sim \mu, a=\pi_f(s)}[ \phi(s,a)]$.
...
(Definition~\ref{def:bc}). We show that linear
...
Recall from Definition~\ref{def:bc}, given $\theta$, we use $\mathcal{T}_h(\theta)\in\mathbb{R}^d$ to represent the linear function resulting from the Bellman backup, i.e.,
\begin{align*}
\forall s,a: \Tcal_h(\theta)^{\top} \phi(s,a)  = r(s,a ) + \E_{s'\sim P_h(\cdot | s,a)} [ \max_{a'} \theta^{\top}\phi(s',a')].
...
\E_{s_h,a_h\sim d^{\pi_f}_h} [\phi(s_h,a_h) ]   \right\rangle.
\end{align*} 
Taking $W_h(g):= w_h -
\mathcal{T}_h (w_{h+1})$ (and note that $W_h(f^\star) = 0$ due to the Bellman optimality condition) and $X_h(f) := \E_{s_h,a_h\sim
...
\begin{align*}
&\E_{s,a\sim d^{\pi_f}_h} \left[ \theta_h^{\top} \phi(s,a) - r(s,a) - \mathbb{E}_{s'\sim P_h(\cdot | s,a)}[ w_h^{\top} \psi(s')  ]  \right] \\
& = \E_{s,a\sim d^{\pi_f}_h} \left[ \theta_h^{\top} \phi(s,a) - Q^\star_h(s,a) + \E_{s'\sim P_h(\cdot | s,a)}[V_h^\star(s') ]- \mathbb{E}_{s'\sim P_h(\cdot | s,a)} [ w_h^{\top} \psi(s')  ]  \right] \\
& = \E_{s,a\sim d^{\pi_f}_h} \left[ (\theta_h - \theta^\star_h)^{\top} \phi(s,a) + ( w_h - w_h^{\star} )^{\top} \E_{s'\sim P_h(\cdot | s,a)}[ \psi(s') ]\right] \\
& = \left\langle \begin{bmatrix} \theta_h - \theta_h^\star \\ w_h - w_h^\star  \end{bmatrix},   \E_{s,a\sim d^{\pi_f}_h} \begin{bmatrix} \phi(s,a) \\ \E_{s'\sim P_h(\cdot | s,a)} [ \psi(s')] \end{bmatrix}  \right\rangle.
\end{align*} 

Taking $W_h(g) :=  \begin{bmatrix} \theta_h  \\
  w_h  \end{bmatrix}$, and $X_h(f) := \begin{bmatrix}
  \phi(s,a) \\ \E_{s'\sim P_h(\cdot | s,a)} [\psi(s')] \end{bmatrix}$,
we have shown that:
...
Chapter~\ref{chap:linear_features}, simply assuming that
...
\begin{align*}
&\E_{s,a \sim d^{\pi_f}_h} \left[ Q_{h,g}(s) - r(s, a) - \E_{s'\sim P_h(\cdot | s, a} [V_{h+1,g}(s')  ] \right] \\
& =   \beta_h(f)^{\top} \left(  \sum_{s,a} \phi_h(s,a) \left[ Q_{h,g}(s,a) - r(s, a) - \E_{s'\sim P_h(\cdot | s,a)} [V_{h+1,g}(s')]   \right]\right),
\end{align*} which shows that $W_h(g) := \sum_{s,a} \phi_h(s,a) \left[ Q_{h,g}(s,a) - r(s, a) - \E_{s'\sim P_h(\cdot | s,a)} [V_{h+1,g}(s')]\right]$ (note that $W_h(f^\star) =0$ due to the Bellman optimality condition), and $X_h(f) := \beta_h(f)$. Note that in this model we can use any hypothesis class $\Hcal$ as long as it contains $f^\star$.
...
\begin{align*}
\E_{s\sim \mu} \left[  V_{0,g}(s) - r(s, \pi_{0,g}(s))   \right] =  \sum_{a\in\Acal}  \E_{s\sim \mu} \left[ \one\{ \pi_g(s) = a \} \left[ Q_{0,g}(s,a) - r(s,a) \right]\right]. 
\end{align*} This means that we can write the Bellman error of $g$ averaged over $\mu$ as an inner product between two vectors --- an all one vector, and a vector whose $a$-th entry is $\E_{s\sim \mu}\left[  \one\{ \pi_g(s) = a \} \left[ Q_{0,g}(s,a) - r(s,a) \right]\right]$, i.e., we have $X_0(f) = \one_{A}$ where $\one_{A}$ represents a A-dim vector will all one , and $W_0(g) \in \mathbb{R}^A$ where the $a$-th entry of $W_0(g)$ is $\E_{s\sim \nu} \one\{\pi_g(s) = a\} [Q_{0,g}(s,a) - r(s,a)]$. Note that $W_0(f^\star) = 0$ where $f^\star$ is the ground truth reward function here.
...
\begin{align}
&\E_{s \sim d^{\pi_f}_h} \left[  V_{h,g}(s) - r(s, \pi_{h,g}(s) ) - \E_{s'\sim P_h(\cdot | s, \pi_{h,g}(s))}[ V_{h+1,g}(s') ] \right] \label{eq:berror_low_rank_mdp}\\
& =  \E_{\tilde{s},\tilde{a} \sim d^{\pi_f}_{h-1}} \E_{s\sim P_{h-1}(\cdot | \tilde{s},\tilde{a})}  \left[  V_{h,g}(s) - r(s, \pi_{h,g}(s) ) - \E_{s'\sim P_h(\cdot | s, \pi_{h,g}(s))} [V_{h+1,g}(s')]  \right]  \nonumber\\
& =  \E_{\tilde{s},\tilde{a} \sim d^{\pi_f}_{h-1}}\int_{s} \mu^\star(s)^{\top} \phi^\star(\tilde{s},\tilde{a}) \left[  V_{h,g}(s) - r(s, \pi_{h,g}(s) ) - \E_{s'\sim P_h(\cdot | s, \pi_{h,g}(s))} [V_{h+1,g}(s') ] \right] d(s) \nonumber \\
& = \left\langle \int_{s} \mu^\star(s)\left[  V_{h,g}(s) - r(s, \pi_{h,g}(s) ) - \E_{s'\sim P_h(\cdot | s, \pi_{h,g}(s))} [V_{h+1,g}(s')]  \right] , \quad  \E_{\tilde{s},\tilde{a}\sim d^{\pi_f}} [\phi^\star(\tilde{s},\tilde{a})  ]   \right\rangle.
\end{align} 
Taking $W_h(g) := \int_{s} \mu^\star(s)\left[  V_{h,g}(s) - r(s,
  \pi_{h,g}(s) ) - \E_{s'\sim P_h(\cdot | s, \pi_{h,g}(s))}
  [V_{h+1,g}(s') ] \right] $ (note that $W_h(f^\star) = 0$ due to the Bellman optimality condition) and $X_h(f) :=
\E_{\tilde{s},\tilde{a}\sim d^{\pi_f}_{h-1}}
...
%\begin{align*}
%&\E_{s \sim d^{\pi_f}_h} \left[  V_{h,g}(s) - r(s, \pi_{h,g}(s) ) - \E_{s'\sim P_h(\cdot | s, \pi_{h,g}(s))}[ V_{h+1,g}(s') ] \right] \\
%& = \left\langle \int_{s} \mu^\star_{h-1}(s)\left[  V_{h,g}(s) - r(s, \pi_{h,g}(s) ) - \E_{s'\sim P_h(\cdot | s, \pi_{h,g}(s))} [V_{h+1,g}(s')]  \right] , \quad  \E_{\tilde{s},\tilde{a}\sim d^{\pi_f}_{h-1}} [\phi_{h-1}^\star(\tilde{s},\tilde{a})  ]   \right\rangle \\
%& = \sum_{i\in\mathcal{K}^\star} \E_{\tilde{s},\tilde{a}\sim d^{\pi_f}_{h-1}} [\phi_{h-1, i }^\star(\tilde{s},\tilde{a})  ]  \int_{s} \mu^\star_{h-1,i}(s)\left[  V_{h,g}(s) - r(s, \pi_{h,g}(s) ) - \E_{s'\sim P_h(\cdot | s, \pi_{h,g}(s))} [V_{h+1,g}(s')]  \right], 
...
SK: I think we should move this to the appendix.
...
\begin{align}
\left\lvert \E_{s\sim d^{\pi_f}_h, a\sim \pi_{est,h}(s;f), s'\sim P_h(\cdot | s,a)} [\ell_f(s,a,s', h, g)]  \right\rvert = \left\lvert \langle W_h(g) - W_h(f^\star), X_h(f) \rangle \right\rvert,\label{eq:bilinear_condition_2}
...
\begin{align*}
\E_{s,a \sim d^{\pi_f}_h,s'\sim P_h(\cdot | s,a)} \ell_f(s,a,s', h,g) & = \E_{s,a\sim d^{\pi_f}_h} \left[\sum_{\tilde{s}} \theta_h^{\top} \phi(s,a,\tilde{s})   V_{h+1,f}(\tilde{s})  - \sum_{\tilde{s}} (\theta^\star_h)^{\top} \phi(s,a,\tilde{s})V_{h+1,f}(s') \right] \\
& = \left(\theta_h - \theta_h^\star  \right)^{\top} \left( \E_{s,a\sim d^{\pi_f}_h} \left[ \sum_{s'} \phi(s,a,s')\right] \right).
\end{align*}  Thus we have shown that $W_h(g) = \theta_h $, and $X_h(f) = \E_{s,a\sim d^{\pi_f}_h} \left[ \sum_{s'} \phi(s,a,s')\right]$.
...
\For{$t = 0\dots T-1 $}
	\State Set $f_t$ as the solution of the following program:
		\begin{align*}
			...
			& \sum_{i=0}^{t-1} \left( \E_{\Dcal_{i,h}} \ell_{f_i}( s,a,s', g ) \right)^2 \leq R^2, \forall h\in [0,\dots, H-1]
		\end{align*}
...
$$\sup_{g\in\Hcal} \big\lvert   \E_{\Dcal} [ \ell_f(s,a,s', h,g)] -
...
$1-\delta$, we have:
\begin{align*}
%\E_{s_0\sim\mu}[V^\star_0(s_0)] - \E_{s_0\sim\mu} [V^{\pi}_{0}(s_0)] \leq   6 \varepsilon_{gen}\sqrt{ d H^3\ln\left(4Hd\left(\frac{B^2_X B_W^2
...
\begin{align*}
\sup_{g\in\Hcal}\left\lvert \E_{\Dcal_{t,h}}[ \ell_{f_t}(s,a,s', g)]  - \E_{s\sim d_h^{\pi_{f_t}}, a\sim \pi_{est}(s), s'\sim P_h(\cdot|s,a)}\left[ \ell_{f_t}(s,a,s', g)\right] \right\rvert \leq \varepsilon_{gen}.
\end{align*}
\end{lemma}
...
\end{lemma}
\begin{proof}
  Based on the second condition in the Bilinear class (Eq.~\ref{eq:bilinear_condition_2}), we have that for $f^\star$:$$| \E_{s\sim d_h^{\pi_{f_t}}, a\sim \pi_{est}(s), s'\sim P_h(\cdot|s,a)}\left[ \ell_{f_t}(s,a,s', f^\star)\right] |  =   | \langle W_h(f^\star ) - W_h(f^\star), X_h(f_t)  \rangle |  = 0,  \forall t.$$
  %  has satisfies the Bellman optimality equations,
%  $\E_{s'\sim P_h(\cdot | s,a)} \ell_f(s,a,s',f^\star) = 0$ for all
  %$s,a$ and $f$.  
  This and our assumption that the event in
  Lemma~\ref{lemma:high_prob_event_brank} holds imply that:
\begin{align*}
\big|\E_{\Dcal_{t,h}} [\ell_{f_t}(s,a,s', f^\star)]\big| & = 
\big|\E_{\Dcal_{t,h}} [\ell_{f_t}(s,a,s', f^\star)]
-\E_{s\sim d_h^{\pi_{f_t}}, a\sim \pi_{est}(s), s'\sim P_h(\cdot|s,a)}\left[ \ell_{f_t}(s,a,s', f^\star)\right] \big| \\
\quad & \leq
\sup_{g\in\Hcal}\left\lvert \E_{\Dcal_{t,h}} [ \ell_{f_t}(s,a,s', g)]  
- \E_{s\sim d_h^{\pi_{f_t}}, a\sim \pi_{est}(s), s'\sim P_h(\cdot|s,a)}\left[ \ell_{f_t}(s,a,s', g)\right] \right\rvert \\
&\leq \varepsilon_{gen}
\end{align*} 
This implies $\sum_{i=0}^{t-1} \left( \E_{\Dcal_{i,h}} \ell_{f_i}( s,a,s',
...
\iffalse
\begin{proof}
The event in Lemma~\ref{lemma:high_prob_event_brank} implies that for all $t$, $$\sup_{g\in\Hcal}\left\lvert \E_{\Dcal_{t,h}} [ \ell(s,a,s', g)]  - \E_{s\sim d_h^{\pi_{f_t}}, a\sim \pi_{est}(s), s'\sim P_h(\cdot|s,a)}\left[ \ell(s,a,s', g)\right] \right\rvert \leq \varepsilon_{gen}.$$

Thus, for $f^\star \in \Hcal$, we have:
\begin{align*}
\E_{\Dcal_{t,h}} [\ell(s,a,s', f^\star)] \leq \varepsilon_{gen} +  \E_{s\sim d_h^{\pi_{f_t}}, a\sim \pi_{est}(s), s'\sim P_h(\cdot|s,a)}\left[ \ell(s,a,s', f^\star)\right] = \varepsilon_{gen},
\end{align*} where in the last equality we use the fact that $\E_{s'\sim P_h(\cdot | s,a)} \ell(s,a,s',f^\star) = 0$ for all $s,a$, since $f^\star$ has zero Bellman error at any $s,a$.
...
\begin{align*}
&  V_0^\star(s_0)   -  V^{\pi_{f_t}}_0(s_0)   \leq  V_{0,f_t}(s_0)  - \E_{s_0\sim \mu} [  V^{\pi_{f_t}}_0(s_0) ]  \\
& = Q_{0,f_t}(s_0,a_0)   - \E\left[ \sum_{h=0}^{H-1} r(s_h,a_h) | \pi_{f_t} \right]\\
& =  \E\left[ \sum_{h=0}^{H-1} Q_{h, f_t}(s_h,a_h) - r(s_h,a_h) + Q_{h+1,f_t}(s_{h+1}, a_{h+1})  \vert \pi_{f_t}   \right]\\
& \leq \sum_{h=0}^{H-1} \left\lvert \E_{s_h,a_h\sim d_h^{\pi_{f_t}} } \left[ Q_{h,f_t}(s_h,a_h) - r(s_h,a_h) - \E_{s_{h+1}\sim P_h(\cdot | s,a)} \left[ V_{h+1,f_t}(s_{h+1}) \right] \right]\right\rvert \\
...
\begin{align*}
& \sum_{i=0}^{t-1 } \left(  \E_{s\sim d_h^{\pi_{f_i}},a\sim \pi_{est}(s), s'\sim P_h(\cdot | s,a)} [\ell_{f_i}(s,a,s', f_t)  ] \right)^2   \leq 2 \sum_{i=0}^{t-1 } \left(  \E_{\Dcal_{i,h}} [ \ell_{f_i}(s,a,s', f_t) ]  \right)^2 \\
& \quad + 2 \sum_{i=0}^{t-1} \left(  \E_{s\sim d_h^{\pi_{f_t}},a\sim
  \pi_{est}(s), s'\sim P_h(\cdot | s,a)} [ \ell_{f_i}(s,a,s', f_t) ]
  -\E_{\Dcal_{i,h}} [\ell_{f_i}(s,a,s', f_t)  ]  \right)^2 
...
&= \sum_{i=0}^{t-1 } \left(  \E_{s\sim d_h^{\pi_{f_i}},a\sim \pi_{est}(s), s'\sim P_h(\cdot | s,a)} [ \ell_{f_i}(s,a,s', f_t) ]  \right)^2 
+ \lambda \| W_h(f_t) - W_h(f^\star) \|_2^2
...
\begin{align*}
\sum_{i=0}^{t-1 } \left(  \E_{s\sim d_h^{\pi_{f_i}},a\sim \pi_{f_i}(s), s'\sim P_h} [ \ell(s,a,s', f_t) ]  \right)^2 & = \sum_{i=0}^{t-1} \left( W_h(f_t)^\top X_h(f_i) \right)^2 \\
...
\begin{align*}
\sum_{i=0}^{t-1 } \left(  \E_{\Dcal_{i,h}} [ \ell(s,a,s', f_t) ]  \right)^2 \leq R^2 = T \varepsilon^2_{gen}, 
\end{align*} which also implies that:
\begin{align*}
& \sum_{i=0}^{t-1 } \left(  \E_{s\sim d_h^{\pi_{f_i}},a\sim \pi_{est}(s), s'\sim P_h} [\ell(s,a,s', f_t)  ] \right)^2   \leq 2 \sum_{i=0}^{t-1 } \left(  \E_{\Dcal_{i,h}} [ \ell(s,a,s', f_t) ]  \right)^2 \\
& \quad + 2 \sum_{i=0}^{t-1} \left(  \E_{s\sim d_h^{\pi_{f_t}},a\sim \pi_{est}(s), s'\sim P_h} [ \ell(s,a,s', f_t) ] -\E_{\Dcal_{i,h}} [\ell(s,a,s', f_t)  ]  \right)^2 \leq 2 T \varepsilon_{gen}^2 + 2 T \varepsilon^2_{gen} = 4 T \varepsilon_{gen}^2,
...
\begin{align*}
  &\sum_{i=0}^{t-1 } \left(  \E_{s\sim d_h^{\pi_{f_i}},a\sim \textrm{Unif}_{\Acal} , s'\sim P_h} [ A \one\{a = \pi_{f_t}(s)\} \ell(s,a,s', f_t)  ] \right)^2 \\
&=  \sum_{i=0}^{t-1 } \left(  \E_{s\sim d_h^{\pi_{f_i}},a\sim \pi_{f_t}(s), s'\sim P_h} [ \ell(s,a,s', f_t)]   \right)^2 = \sum_{i=0}^{t-1} \left( W_h(f_t)^\top X_h(f_i) \right)^2 = \left\| W_h(f_t) \right\|^2_{\Sigma_{t, h}} - \lambda \| W_h(f_t) \|_2^2 \leq 4 T\varepsilon^2_{gen}.
\end{align*}
