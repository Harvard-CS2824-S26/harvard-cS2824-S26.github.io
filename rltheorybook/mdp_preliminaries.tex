...
\begin{align*}
V^\pi(s) &= \E_{a\sim \pi(\cdot\mid s)}\!\left[ Q^\pi(s,a)\right], \\
Q^\pi(s,a) &= r(s,a) + \gamma \E_{s' \sim P(\cdot\mid s, a)}\!\left[
V^\pi(s') \right].
\end{align*}
...
\[
Q(s,a) =  r(s,a) + \gamma \E_{s' \sim P(\cdot\mid s, a)}\left[ \max_{a'\in\Acal} Q(s',a') \right].
\]
...
means
\[
Q(s,a)=r(s,a)+\gamma \E_{s'\sim P(\cdot\mid s,a)}[V_Q(s')]
= r(s,a)+\gamma \E_{s'\sim P(\cdot\mid s,a)}[Q(s',\pi(s'))],
\]
...
= \E_{s'\sim P(\cdot\mid s,a)}\!\left[Q^\pi(s',\pi(s'))-Q^\pi(s',\pi'(s'))\right]\ge 0,
\]
...
In terms of finite horizon \emph{Bellman operators}, we can write this
recursion as follows:
for a function $f:\Scal\times\Acal\to\R$, define the stage-$h$ \emph{optimal}
Bellman operator $\Tcal_h$ by
\[
(\Tcal_h f)(s,a)
\;:=\;
r_h(s,a) + \E_{s'\sim P_h(\cdot\mid s,a)}\Big[\,\max_{a'\in\Acal} f(s',a')\,\Big].
\]
Under this definition, the optimal $Q$-functions satisfy the
recursion:
\begin{align*}
  Q^\star_H &\equiv 0\\
  Q^\star_h &= \Tcal_h Q^\star_{h+1} \quad\quad \textrm{for } \, h=H-1,\dots,0/
\end{align*}
...
Q_h(s,a) \;=\; r_h(s,a) \;+\; \E_{s' \sim P_h(\cdot\mid s, a)}\!\left[\max_{a'\in\Acal} Q_{h+1}(s',a')\right].
\end{equation}
...
\begin{tabular}{ |c|c|c| }
\hline
 & \textbf{Poly?} & \textbf{Strongly Poly?} \\ \hline
...
\right\}$ \\ \hline
\textbf{LP Algorithms} &
$|\Scal|^3 |\Acal|\, L(P,r,\gamma)$ &
$|\Scal|^4 |\Acal|^4 \log\frac{|\Scal| }{1-\gamma}$ \\ \hline
\end{tabular}
\caption{Computational complexities of various approaches (universal
  constants are omitted) to find an \emph{exact} solution.
...
= \gamma \E_{s'\sim P(\cdot\mid s,a)}\!\left[V^\star(s') - V^{\pi_Q}(s')\right]
...
\item \emph{Policy evaluation.} Compute the value of the current policy, e.g.,
      ...
      $Q^{\pi_k}(s,a)=r(s,a)+\gamma \E_{s'\sim P(\cdot\mid s,a)}[V^{\pi_k}(s')]$.
...
\[
\Tcal Q^{\pi_k}(s,a)
= r(s,a) + \gamma \E_{s'\sim P(\cdot\mid s,a)}[V_{Q^{\pi_k}}(s')]
\ge r(s,a) + \gamma \E_{s'\sim P(\cdot\mid s,a)}[V^{\pi_k}(s')]
= Q^{\pi_k}(s,a).
\]
...
\[
(\Tcal^{\pi'} V)(s) := r(s,\pi'(s)) + \gamma \E_{s'\sim P(\cdot\mid s,\pi'(s))}[V(s')].
\]
...
\[
(\Tcal^{\pi'} V^{\pi_k})(s)
= r(s,\pi'(s)) + \gamma \E_{s'\sim P(\cdot\mid s,\pi'(s))}[V^{\pi_k}(s')]
= Q^{\pi_k}(s,\pi'(s))
= \max_a Q^{\pi_k}(s,a)
= V_{Q^{\pi_k}}(s)
\ge V^{\pi_k}(s).
...
\[
Q^{\pi_{k+1}}(s,a)
= r(s,a) + \gamma \E_{s'\sim P(\cdot\mid s,a)}[V^{\pi_{k+1}}(s')]
\ge r(s,a) + \gamma \E_{s'\sim P(\cdot\mid s,a)}[V_{Q^{\pi_k}}(s')]
= \Tcal Q^{\pi_k}(s,a),
\]
...
Q_h(s,a)
= r_h(s,a) + \E_{s'\sim P_h(\cdot\mid s,a)}\!\left[\max_{a'\in\Acal} Q_{h+1}(s',a')\right].
\end{equation}
...
Using the identity
\[
V^\pi(\mu) = \E\!\left[\sum_{t=0}^\infty \gamma^t r(s_t,a_t)\right]
...
\[
A^{\pi^\star}(s,a) = Q^{\pi^\star}(s,a)-V^{\pi^\star}(s)\le 0
\]
for all $(s,a)$.

Recall the discounted state-action visitation distribution $d_\mu^\pi$ from
Equation~\ref{eq:dpisa}. (Equivalently, $d_\mu^\pi$ is the distribution on
$\Scal\times\Acal$ defined by the normalized discounted occupancy measure.)
A basic identity we will repeatedly use is that for any function
$f:\Scal\times\Acal\to\R$,
\begin{equation}\label{eq:f_equal}
\E\!\left[\sum_{t=0}^\infty \gamma^t f(s_t,a_t)\,\Big|\,\pi,\,s_0\sim\mu\right]
=\frac{1}{1-\gamma}\;\E_{(s,a)\sim d_\mu^\pi}\!\left[f(s,a)\right].
\end{equation}
This follows immediately from the definition of $d_\mu^\pi$.
...
V^\pi(\mu) - V^{\pi'}(\mu)
=
\frac{1}{1-\gamma}\E_{(s,a)\sim d_\mu^\pi}\!\left[A^{\pi'}(s,a)\right].
\]
\end{lemma}
...
\end{align*}
Applying \eqref{eq:f_equal} with $f=A^{\pi'}$ and averaging over $s_0=s\sim \mu$ gives the stated result.
\end{proof}
