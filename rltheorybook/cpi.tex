...
\begin{align*}
\E_{s\sim d^{\pi}_\mu} A^{\pi}(s, \widehat\pi(s)) \geq \max_{\widetilde\pi\in \Pi } \E_{s\sim d^{\pi}_\mu} A^{\pi}(s, \widetilde\pi(s)) - \varepsilon.
\end{align*}
...
\subsection{The CPI Algorithm}
 ...
 Note we use $\pi_\alpha = (1-\alpha) \pi + \alpha \pi'$ to refer to a randomized policy which at any state $s$, chooses an action according to $\pi$ with probability $1-\alpha$ and according to $\pi'$ with probability $\alpha$. The greedy policy $\pi'$ is computed using the $\varepsilon$-approximate greedy policy selector $\Gcal_{\varepsilon}(\pi^t, \Pi, \mu)$. The algorithm is terminate when there is no significant one-step improvement over $\pi^t$, i.e., $\E_{s\sim d^{\pi^t}_\mu} A^{\pi^t}(s, \pi'(s))) \leq \varepsilon$.
...
\begin{algorithmic}[1]
    ...
    \For{$t=0,1,2\ldots$}
        ...
        %\State Obtain a $Q$ value approximation $\Qh_i$ satisfying~\eqref{eqn:cpi-pre} with parameter $\epsilon/2\sqrt{|\Acal|}$. \label{algline:cpi-eps}
        %\State Define $\pitil_i(s) = \argmax_{a\in\Acal} \Qh_i(s,a)$, and let $\widehat{A}_i = \E_{s\in d^{\pi_i}} \Qh_i(s,\pitil_i(s)) - \Qh_i(s,\pi_i(s))$.
        \If{ $\E_{s\sim d_\mu^{\pi^t}} A^{\pi^t}(s, \pi'(s))  \leq \varepsilon$ }
        		\State Return $\pi^t$
        \EndIf
...
\begin{align*}
V_\mu^{\pi^{t+1}} - V_\mu^{\pi^t} = \frac{1}{1-\gamma} \E_{s\sim d^{\pi^{t+1}}_{\mu}} A^{\pi^t}(s, \pi^{t+1}(s)) =  \frac{ \alpha }{1-\gamma} \E_{s\sim d^{\pi^{t+1}}_{\mu}} A^{\pi^t}(s, \pi'(s)),
\end{align*} where the last equality we use the fact that $\pi^{t+1} = (1-\alpha) \pi^t + \alpha \pi'$ and $A^{\pi^t}(s,\pi^t(s)) = 0$ for all $s$. Thus, if we can search for a policy $\pi'\in \Pi$ that maximizes $\E_{s\sim d^{\pi^{t+1}}_{\mu}} A^{\pi^t}(s, \pi'(s))$ and makes $\E_{s\sim d^{\pi^{t+1}}_{\mu}} A^{\pi^t}(s, \pi'(s)) > 0$, then we can guarantee policy improvement. However, at episode $t$, we do not know the state distribution of $\pi^{t+1}$ and all we have access to is $d^{\pi^t}_\mu$.  Thus, we explicitly make the policy update procedure to be conservative such that $d^{\pi^t}_{\mu}$ and the new policy's distribution $d^{\pi^{t+1}}_\mu$ is guaranteed to be not that different. Thus we can hope that  $\E_{s\sim d^{\pi^{t+1}}_{\mu}} A^{\pi^t}(s, \pi'(s))$ is close to $\E_{s\sim d^{\pi^{t}}_{\mu}} A^{\pi^t}(s, \pi'(s))$, and the latter is something that we can manipulate using the greedy policy selector. 
...
Consider any episode $t$. Denote $\mathbb{A} = \E_{s\sim d^{\pi^t}_\mu} A^{\pi^t}(s, \pi'(s))$. 
We have:
\begin{align*}
...
& \geq \mathbb{E}_{s\sim d^{\pi^{t}}_{\mu}} \alpha A^{\pi^t}(s, \pi'(s))  - \frac{2\alpha^2 \gamma }{ (1-\gamma )^2} = \alpha\left( \mathbb{A} - \frac{2\alpha \gamma}{(1-\gamma)^2} \right)
 \end{align*} where the first inequality we use the fact that for any two distributions $P_1$ and $P_2$ and any function $f$, $| \E_{x\sim P_1}f(x) - \E_{x\sim P_2}f(x) | \leq \sup_{x} |f(x)| \| P_1 - P_2 \|_1 $,  for the second inequality, we use the fact that $| A^{\pi}(s,a)| \leq 1/(1-\gamma)$ for any $\pi,s,a$, and the last inequality uses Lemma~\ref{lem:policy_deviation_to_state_deviation}.
...
\begin{theorem}[Global optimality of CPI]
    ...
    \end{align*} where $\epsilon_{\Pi} := \E_{s\sim d^{\pi}_\mu}
    [\max_{a\in\Acal } A^{\pi}(s, a)]  - \max_{\pi\in \Pi }\E_{s\sim
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &\leq \frac{1}{1-\gamma}\E_{s\sim d^{\pi^\star}} \max_{a\in\Acal} A^\pi(s,a)\\
        &\leq \frac{1}{1-\gamma} \norm{\frac{d^{\pi^*}}{d_{\mu}^\pi}}_\infty \E_{s\sim d^\pi_\mu} \max_{a\in\Acal} A^\pi(s,a)\\
        & \leq \frac{1}{(1-\gamma)^2} \norm{\frac{d^{\pi^*}}{\mu}}_\infty \E_{s\sim d^\pi_\mu} \max_{a\in\Acal} A^\pi(s,a) \\
        & = \leq \frac{1}{(1-\gamma)^2} \norm{\frac{d^{\pi^*}}{\mu}}_\infty \left[ \max_{\hat\pi\in \Pi}\E_{s\sim d^\pi_\mu} A^\pi(s,\hat\pi(s)) -  \max_{\hat\pi\in \Pi}\E_{s\sim d^\pi_\mu} A^\pi(s,\hat\pi(s)) + \E_{s\sim d^\pi_\mu} \max_{a\in\Acal} A^\pi(s,a)   \right] \\
...
\begin{align}
\label{eq:exact_API}
\pi^{t+1} = \argmax_{\pi \in \Pi} \E_{s\sim d^{\pi^t}_\mu} A^{\pi^t}(s, \pi(s)).
\end{align}
\begin{claim}
...
\begin{align*}
\pi \in \argmax_{\pi\in \Pi} \E_{s\sim d^{\pi'}_\mu} A^{\pi'}(s, \pi(s)).
\end{align*}
...
Similarly, one can argue that $\pi' \in \argmax_{\pi \in \Pi} \E_{s \sim d^{\pi''}_h}A^{\pi''}(s,\pi(s))$, i.e., at $\pi''$, API will switch back to $\pi'$ in the next iteration. 
...
\begin{align*}
\mathbb{A} := \max_{\pi\in \Pi} \E_{s\sim d^{\pi'}_\mu} A^{\pi'}(s, \pi(s)) > 0,
...
\begin{align*}
V^{\pi^{t+1}}_{\mu} - V_{\mu}^{\pi^t} & = \frac{1}{1-\gamma}\E_{s\sim d_{\mu}^{\pi^{t+1}}} A^{\pi^t}(s, \pi^{t+1}(s)) = \frac{1}{1-\gamma}\E_{s\sim d_{\mu}^{\pi^{t+1}}} \max_{a\in\Acal} A^{\pi^t}(s, a)\\
 & =  \frac{1}{1-\gamma} \E_{s\sim d^{\pi^t}_\mu} \frac{ d_\mu^{\pi^{t+1}(s)} }{d^{\pi^t}_\mu(s)} \max_{a\in\Acal} A^{\pi^t}(s, a) \geq  \inf_{s}  \frac{ d_\mu^{\pi^{t+1}(s)} }{d^{\pi^t}_\mu(s)}  \frac{1}{1-\gamma} \E_{s\sim d^{\pi^t}_\mu} \max_{a\in\Acal} A^{\pi^t}(s,a)
\end{align*} which leads to:
\begin{align*}
V^{\pi^{t+1}}_{\mu} - V_{\mu}^{\pi^t}  & \geq  \frac{1}{(1-\gamma)^2} \inf_{s} \frac{ \mu(s) }{  d^{\pi^t}_{\mu}(s) } \E_{s\sim d^{\pi^t}_\mu} \max_{a\in\Acal} A^{\pi^t}(s,a) = \frac{1}{(1-\gamma)^2 \| d^{\pi^t}_\mu / \mu  \|_{\infty}} \E_{s\sim d^{\pi^t}_\mu} \max_{a\in\Acal} A^{\pi^t}(s,a) \\
...
\begin{align*} 
 &\max_{\theta} \E_{s\sim d^{\pi^{\theta_t}}_\mu} \E_{a\sim \pi_\theta(\cdot | s)} A^{\pi^{\theta_t}}(s, a) \\
...
\begin{align}
 &\max_{\theta} \left\langle \E_{s\sim d^{\pi^{\theta_t}}_\mu} \E_{a\sim \pi_{\theta_t}(\cdot | s)} \nabla_\theta \ln \pi_{\theta_t}(a|s) A^{\pi^{\theta_t}}(s, a), \theta\right\rangle  \\
...
&=  \sum_{\tau} {\Pr}^{\pi^{\theta_t}}_{\mu}(\tau) \left( \sum_{h=0}^{H-1} \ln \frac{ \pi_{\theta_t}(a_h|s_h)}{\pi_{\theta}(a_h|s_h)} \right) = \sum_{h=0}^{H-1} \E_{s_h,a_h\sim \mathbb{P}_h^{\pi_{\theta_t}}} \ln\frac{ \pi_{\theta_t}(a_h|s_h)  }{ \pi_\theta(a_h|s_h)}.
\end{align*}
Thus, for $\nabla_\theta KL\left(  {\Pr}^{\pi^{\theta_t}}_{\mu}  || {\Pr}^{\pi_{\theta}}_{\mu}    \right)$, we have:
\begin{align*}
\nabla KL\left(  {\Pr}^{\pi^{\theta_t}}_{\mu}  || {\Pr}^{\pi_{\theta}}_{\mu}    \right) |_{\theta = \theta_t} &= \sum_{h=0}^{H-1 } \E_{s_h,a_h\sim \mathbb{P}_h^{\pi_{\theta_t}}}  - \nabla \ln \pi_{\theta_t}(a_h|s_h)\\
& =  - \sum_{h=0}^{H-1 } \E_{s_h \sim \mathbb{P}_h^{\pi_{\theta_t}}}  \E_{a_h\sim \pi_{\theta_t}(\cdot |  s_h)} \nabla \ln \pi_{\theta_t}(a_h|s_h) = 0,
...
\begin{align*}
\nabla^2 KL\left(  {\Pr}^{\pi^{\theta_t}}_{\mu}  || {\Pr}^{\pi_{\theta}}_{\mu}    \right) |_{\theta = \theta_t}  & =  - \sum_{h=0}^{H-1 } \E_{s_h,a_h\sim \mathbb{P}_h^{\pi_{\theta_t}}}  \nabla^2 \ln \pi_{\theta}(a_h|s_h) |_{\theta = \theta_t} \\
& =  - \sum_{h=0}^{H-1 } \E_{s_h,a_h\sim \mathbb{P}_h^{\pi_{\theta_t}}}  \left( \nabla \left( \frac{ \nabla \pi_{\theta}(a|s) }{ \pi_{\theta}(a|s) }  \right) \right) \\
& = - \sum_{h=0}^{H-1 } \E_{s_h,a_h\sim \mathbb{P}_h^{\pi_{\theta_t}}}  \left(  \frac{\nabla^2 \pi_\theta(a_h|s_h)}{ \pi_{\theta}(a_h|s_h) }    - \frac{ \nabla \pi_\theta(a_h|s_h) \nabla\pi_\theta(a_h|s_h)^{\top}  }{ \pi^2_\theta(a_h|s_h) }   \right) \\
& = \sum_{h=0}^{H-1 } \E_{s_h,a_h\sim \mathbb{P}_h^{\pi_{\theta_t}}} \nabla_\theta \ln \pi_\theta(a_h|s_h) \nabla_\theta \ln \pi_\theta(a_h|s_h) ^{\top},
\end{align*} where in the last equation we use the fact that $\E_{s_h,a_h\sim \mathbb{P}_h^{\pi_{\theta_t}}} \frac{\nabla^2 \pi_\theta(a_h|s_h)}{ \pi_{\theta}(a_h|s_h) } = 0 $.
\end{proof}
...
\begin{align}
 &\max_{\theta} \E_{s\sim d^{\pi^{\theta_t}}_\mu} \E_{a\sim \pi^\theta(\cdot | s)} A^{\pi^{\theta_t}}(s, a)  \label{eq:ppo_obj}\\
...
\begin{align}
\max_{\theta} \E_{s\sim d^{\pi^{\theta_t}}_\mu} \E_{a\sim \pi^{\theta_t}(\cdot | s)}  \frac{\pi^{\theta}(a|s)}{ \pi^{\theta_t}(\cdot | s) }A^{\pi^{\theta_t}}(s, a), 
\label{eq:IS_obj_ppo}
\end{align} where we can easily approximate the expectation $\E_{s\sim d^{\pi^{\theta_t}}_\mu} \E_{a\sim \pi^{\theta_t}(\cdot | s)} $ via finite samples $s\sim d^{\pi^{\theta_t}}, a\sim \pi^{\theta_t}(\cdot | s)$.
...
\begin{align}
L(\theta): =   \E_{s\sim d^{\pi^{\theta_t}}_\mu} \E_{a\sim \pi^{\theta_t}(\cdot | s)}   \min\left\{  \frac{\pi^{\theta}(a|s)}{ \pi^{\theta_t}(\cdot | s) }  A^{\pi^{\theta_t}}(s,a), \text{clip}\left( \frac{\pi^{\theta}(a|s)}{ \pi^{\theta_t}(\cdot | s) }; 1-\epsilon, 1+\epsilon \right) A^{\pi^{\theta_t}}(s,a)  \right\},
\end{align}
...
\begin{align*}
\max_{\theta} \left\langle (\theta - \theta^t), \; \E_{s,a\sim d^{\pi^{\theta_t}}_\mu} \nabla_{\theta} \ln\pi^{\theta_t}(a|s) A^{\pi^{\theta_t}}(s,a)      \right\rangle,
