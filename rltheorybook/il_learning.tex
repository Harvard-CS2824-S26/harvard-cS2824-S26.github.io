...
\end{theorem}
\begin{proof}
We start with the performance difference lemma and the fact that $\E_{a\sim \pi(\cdot | s)}A^{\pi}(s,a) = 0$:
\begin{align*}
&(1-\gamma) \left(V^{\star} - V^{\widehat\pi}\right) = \mathbb{E}_{s\sim d^{\pi^\star}} \E_{a\sim \pi^\star(\cdot | s)}A^{\widehat\pi}(s,a) \\
& =   \mathbb{E}_{s\sim d^{\pi^\star}} \E_{a\sim \pi^\star(\cdot | s)}A^{\widehat\pi}(s,a) - \mathbb{E}_{s\sim d^{\pi^\star}} \E_{a\sim \widehat\pi(\cdot | s)}A^{\widehat\pi}(s,a) \\
& \leq \E_{s\sim d^{\pi^\star}} \frac{1}{1-\gamma }\left\| \pi^\star(\cdot | s) - \widehat\pi(\cdot | s)   \right\|_1 \\
& \leq \frac{1}{1-\gamma} \sqrt{  \E_{s\sim d^{\pi^\star}}\left\| \pi^\star(\cdot | s) - \widehat\pi(\cdot | s)   \right\|^2_1}\\
& = \frac{1}{1-\gamma} \sqrt{ 4 \E_{s\sim d^{\pi^\star}}\left\| \pi^\star(\cdot | s) - \widehat\pi(\cdot | s)   \right\|^2_{tv}}.
\end{align*} where we use the fact that $\sup_{s,a,\pi} | A^{\pi}(s,a) | \leq \frac{1}{1-\gamma}$, and the fact that $(\E[x])^2 \leq \E[x^2]$.
...
\begin{align*}
f_{\pi,\pi'} := \argmax_{f: \|f\|_{\infty} \leq 1}\left[ \E_{s,a\sim d^{\pi}} f(s,a) - \E_{s,a\sim d^{\pi'}}f(s,a) \right].
...
\begin{align*}
\max_{f\in \Fcal} \left[ \mathbb{E}_{s,a\sim d^{\pi}} f(s,a) - \E_{s,a\sim d^{\star}} f(s,a)  \right]  &  = \max_{f: \|f\|_{\infty}\leq 1} \left[ \mathbb{E}_{s,a\sim d^{\pi}} f(s,a) - \E_{s,a\sim d^\star}f(s,a)  \right]  =  \left\|  d^{\pi} - d^{\pi^\star}   \right\|_{1},
\end{align*} where the first equality comes from the fact that $\Fcal$ includes  $\arg\max_{f: \|f\|_{\infty}\leq 1} \left[ \mathbb{E}_{s,a\sim d^{\pi}} f(s,a) - \E_{s,a\sim d^\star}f(s,a)  \right]$.

Via Hoeffding's inequality and a union bound over $\Fcal$, we get that with probability at least $1-\delta$, for all $f\in\Fcal$:
\begin{align*}
\left\lvert  \frac{1}{M} \sum_{i=1}^M f(s^\star_i,a^\star_i) - \E_{s,a\sim d^\star}f(s,a)  \right\rvert \leq 2\sqrt{ \frac{ \ln(|\Fcal |/\delta) }{M} } := \epsilon_{stat}.
\end{align*}
Denote $\widehat{f} := \arg\max_{f\in \Fcal} \left[ \mathbb{E}_{s,a\sim d^{\widehat\pi}} f(s,a) - \E_{s,a\sim d^{\star}} f(s,a)  \right] $, and $\widetilde{f}:= \arg\max_{f\in\Fcal} \mathbb{E}_{s,a\sim d^{\widehat\pi}} {f}(s,a) - \frac{1}{M} \sum_{i=1}^M {f}(s_i,a_i)$.
Hence, for $\widehat\pi$, we have:
\begin{align*}
\left\| d^{\widehat\pi} - d^\star  \right\|_{1} & = \mathbb{E}_{s,a\sim d^{\widehat\pi}} \widehat{f}(s,a) - \E_{s,a\sim d^{\star}} \widehat{f}(s,a)  \leq \mathbb{E}_{s,a\sim d^{\widehat\pi}} \widehat{f}(s,a) - \frac{1}{M} \sum_{i=1}^M \widehat{f}(s^\star_i,a^\star_i) + \epsilon_{stat} \\
...
& \leq \mathbb{E}_{s,a\sim d^{\pi^\star}} \widetilde{f}(s,a) - \E_{s,a\sim d^\star} \widetilde{f}(s,a) +2 \epsilon_{stat}  = 2\epsilon_{stat},
\end{align*} where in the third inequality we use the optimality of $\widehat\pi$. 

Recall that $V^{\pi} = \E_{s,a\sim d^{\pi}} r(s,a) / (1-\gamma)$, we have:
\begin{align*}
V^{\widehat\pi} - V^\star = \frac{1}{1-\gamma} \left( \E_{s,a\sim d^{\widehat\pi}}r(s,a) - \E_{s,a\sim d^\star}r(s,a) \right) \leq \frac{\sup_{s,a} |r(s,a)| }{1-\gamma} \left\| d^{\widehat\pi} - d^\star \right\|_1 \leq \frac{2}{1-\gamma} \epsilon_{stat}. 
\end{align*} 
...
\begin{align*}
&\widetilde{f} = \argmax_{f\in\Fcal} \left[ \E_{s,a\sim d^{\widehat\pi}} f(s,a) - \E_{s,a\sim d^{\widetilde\pi}} f(s,a) \right], \\
& \overline{f} = \argmax_{f\in \Fcal} \left[ \E_{s,a\sim d^{\widehat\pi}} f(s,a) - \frac{1}{M}\sum_{i=1}^M f(s^\star_i,a^\star_i) \right], \\
& f' = \arg\max_{f\in\Fcal} \left[\E_{s,a\sim d^{\widetilde\pi}} \left[ {f}(s,a)\right] -  \frac{1}{M}\sum_{i=1}^M {f}( s_i^\star, a_i^\star ) \right] .
...
& =  \E_{s,a\sim d^{\widehat\pi}} \left[ \widetilde{f}(s,a)\right] - \E_{s,a\sim d^{\widetilde\pi}}\left[ \widetilde{f}(s,a)\right]   + \left\| d^{\widetilde\pi} - d^{\pi^\star} \right\|_1  \\
&  =  \E_{s,a\sim d^{\widehat\pi}} \left[ \widetilde{f}(s,a)\right] -  \frac{1}{M}\sum_{i=1}^M \widetilde{f}( s_i^\star, a_i^\star )  + \frac{1}{M}\sum_{i=1}^M \widetilde{f}( s_i^\star, a_i^\star ) -
\E_{s,a\sim d^{\widetilde\pi}}\left[ \widetilde{f}(s,a)\right]   + \left\| d^{\widetilde\pi} - d^{\pi^\star} \right\|_1 \\
& \leq  \E_{s,a\sim d^{\widehat\pi}} \left[ \overline{f}(s,a)\right] -  \frac{1}{M}\sum_{i=1}^M \overline{f}( s_i^\star, a_i^\star )  + \frac{1}{M}\sum_{i=1}^M \widetilde{f}( s_i^\star, a_i^\star ) - \E_{s,a\sim d^\star} \widetilde{f}(s,a) \\
& \qquad + \left[ \E_{s,a\sim d^\star} \widetilde{f}(s,a)-
\E_{s,a\sim d^{\widetilde\pi}}\left[ \widetilde{f}(s,a)\right]  \right] + \left\| d^{\widetilde\pi} - d^{\pi^\star} \right\|_1 \\
& \leq \E_{s,a\sim d^{\widetilde\pi}} \left[ {f'}(s,a)\right] -  \frac{1}{M}\sum_{i=1}^M {f'}( s_i^\star, a_i^\star ) +   2 \sqrt{\frac{\ln(|\Fcal|/\delta)}{ M }} + 2\| d^{\widetilde\pi} - d^\star\|_1  \\
& \leq \E_{s,a\sim d^{\widetilde\pi}} \left[ {f'}(s,a)\right] -  \E_{s,a\sim d^{\star}}\left[ f'( s, a ) \right] + 4 \sqrt{\frac{\ln(|\Fcal|/\delta)}{ M }} + 2\| d^{\widetilde\pi} - d^\star\|_1 \\
& \leq 3 \| d^\star - d^{\widetilde\pi} \|_1 + 4 \sqrt{ \frac{\ln(|\Fcal|/\delta)}{M } },
\end{align*} where the first inequality uses the definition of $\overline{f}$, the second inequality uses the fact that $\widehat\pi$ is the minimizer of $\max_{f\in\Fcal} \E_{s,a\sim d^{\pi}}f(s,a) - \frac{1}{M}\sum_{i=1}^M f(s_i^\star,a_i^\star)$, along the way we also use Heoffding's  inequality where $\forall f\in\Fcal$, $| \E_{s,a\sim d^\star} f(s,a) - \sum_{i=1}^M f(s_i^\star,a_i^\star) | \leq 2\sqrt{ \ln(|\Fcal|/\delta) / M}$, with probability at least $1-\delta$.
\end{proof}
...
The goal of the learner is to compute a policy $\pi: \Scal\mapsto \Delta(A)$ such that when measured under the true cost function, it performs as good as the expert, i.e., $\E_{s,a\sim d^{\pi}} \theta^\star\cdot \phi(s,a) \approx \E_{s,a\sim d^{\pi^\star}} \theta^\star\cdot \phi(s,a)$.
...
\begin{align*}
Q^\star_h(s,a) = r(s,a) + \E_{s'\sim P(\cdot | s,a)} V^\star_{h+1}(s').
\end{align*} For $V^\star_h$, we have:
\begin{align*}
V^{\star}_{h}(s) = \min_{\rho\in\Delta(\Acal)} \sum_{a} \rho(a) \left( c(s,a)  +  \ln \rho(a)   + \E_{s'\sim P(\cdot|s,a)}V^\star_{h+1}(s')\right).
...
\begin{algorithmic}[1]
	...
	\State Policy update: $\rho_{t+1} = \argmax_{\rho\in \Delta(|\Pi|)} \sum_{j=0}^t \sum_{i=1}^{|\Pi|} \rho[i] \left(\E_{a\sim\pi_i(\cdot|s_t)}A^\star(s_t,a)\right) - \lambda \sum_{i=1}^{|\Pi|} \rho[i] \ln(\rho[i])$
    \EndFor
\end{algorithmic}
\caption{AggreVaTe}
\label{alg:AggreVaTe}
...
To analyze the algorithm, let us introduce some additional notations. Let us denote $\ell_t(\rho) = \sum_{i=1}^{|\Pi|} \rho[i] \E_{a\sim \pi_i(\cdot | s)} A^\star(s_t, a)$, which is dependent on state $s_t$ generated at iteration $t$, and is a linear function with respect to decision variable $\rho$. AggreVaTe is essentially running the specific online learning algorithm, Follow-the-Regularized Leader (FTRL) (e.g., see \cite{shalev2011online}) with Entropy regularization:
\begin{align*}
...
\begin{align*}
\tilde\ell_t(\rho_t) =  \E_{s\sim d^{\pi_{\rho_t}}} \sum_{i=1}^{|\Pi|} \rho_t[i] \E_{a\sim \pi_i(\cdot | s)} A^\star(s,a)
\end{align*} Denote $\E_t[ \ell_t(\rho_t)]$ as the conditional expectation of $\ell_t(\rho_t)$, conditioned on all history up and including the end of iteration $t-1$. Thus, we have $\E_{t}[\ell_t](\rho_t) = \tilde\ell_t(\rho_t)$ as $\rho_t$ only depends on the history up to the end of iteration $t-1$.  Also note that $|\ell_t(\rho)| \leq c$. Thus by Azuma-Hoeffding inequality (Theorem~\ref{thm:Azuma}), we get:
\begin{align*}
...
\begin{align*}
(1-\gamma)\left( - V^{\star} + V^{\pi_{\rho_{\hat{t}}}}\right) & = \E_{s\sim d^{\pi_{\rho_{\hat{t}}}}} \sum_{i=1}^{|\Pi|}\rho_{\hat{t}}[i] \E_{a\sim \pi_i(\cdot | s)} A^\star(s,a) = \ell_{\hat{t}} (\rho_{\hat{t}}) \\
...
\begin{align*}
V^\star(s)_{H-1} = \min_{\pi_{H-1}(\cdot | s) \in \Delta(\Acal)} \E_{a\sim \rho}\left[   \frac{1}{2} s^{\top} Q x + \frac{1}{2} a^{\top} R a +  \beta \ln \rho(a)   \right]
\end{align*}
...
\begin{align*}
\E_{s \sim d^{\widehat\pi}} \left\| \widehat\pi(\cdot | s) - \pi^\star(\cdot | s)  \right\|_{tv} \leq  \epsilon. 
\end{align*}
Via performance difference lemma, this immediately implies that:
\begin{align*}
(1-\gamma)\left(V^{\star} - V^{\widehat\pi}\right)  & =  - \E_{s\sim d^{\widehat\pi}} \E_{a\sim \widehat\pi(\cdot | s)} A^{\star}(s,a)  = - \E_{s\sim d^{\widehat\pi}} \E_{a\sim \widehat\pi(\cdot | s)} A^{\star}(s,a)  +  \E_{s\sim d^{\widehat\pi}} \E_{a\sim \pi^\star(\cdot | s)} A^{\star}(s,a) \\
& \leq  \left( \sup_{s,a} \left\lvert A^\star(s,a) \right\lvert \right)  \E_{s\sim \widehat\pi} \left\| \widehat\pi(\cdot | s) - \pi^\star(\cdot | s) \right\|_{1}.
\end{align*}
Thus, we have:
\begin{align*}
V^{\star} - V^{\widehat\pi} \leq \frac{ \sup_{s,a} | A^\star(s,a) |  }{1-\gamma} \epsilon. 
\end{align*}
DAgger avoids the quadratic effective horizon dependency $1/(1-\gamma)^2$ only under the condition that $\E_{s\sim d^{\widetilde\pi}} \max_{a} | A^\star(s,a) | = o\left( \frac{1}{1-\gamma} \right)$. Note that in the worst case, it is possible that $\E_{s\sim d^{\widetilde\pi}} \max_{a} | A^\star(s,a) | = \Theta\left( 1/ (1-\gamma)\right)$, in which case unlike DM-ST, DAgger indeed cannot avoid the quadratic dependency on the effective horizon and also suffers distribution shift issue as the Behavior cloning algorithm. Cooking up such MDPs is not hard. 
