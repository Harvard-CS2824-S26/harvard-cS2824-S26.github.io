...
\item For real-valued matrix $A$, we denote $\|A\|_2 = \sup_{x:\|x\|_2
    = 1} \|Ax\|_2$ which denotes the maximum singular value of $A$.
  We denote $\|A\|_F$ as the Frobenius norm $\|A\|_F^2 = \sum_{i,j} A_{i,j}^2$ where $A_{i,j}$ denotes the $i,j$'th entry of $A$.
  
\item For a vector $x$ and a matrix $M$, unless stated otherwise, $\|x\|$ and $\|M\|$
  denote the Euclidean and spectral norms, respectively. For a
  positive semi-definite matrix $M$, we use $\|x\|_M :=
...
\item For a policy $\pi$ and initial distribution $\mu$, the \emph{discounted state-action occupancy measure} $d_\mu^\pi \in \Delta(\Scal\times\Acal)$ is defined as:
\[
d_\mu^\pi(s,a) := (1-\gamma)\sum_{t=0}^\infty \gamma^t {\Pr}^\pi(s_t=s, a_t=a \mid s_0 \sim \mu).
\]
We similarly define the state occupancy measure $d_\mu^\pi(s) := \sum_{a\in\Acal} d_\mu^\pi(s,a)$.
