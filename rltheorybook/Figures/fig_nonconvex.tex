	...
	\begin{figure}
		\centering
			...
			\node[state, accepting, above of=s2] (s4) {$s_4$};
			\node[state, accepting, right of=s2] (s5) {$s_5$};
			\node[state, accepting, above of=s1] (s3) {$s_3$};
			\draw (s4) edge[loop above] node{$0$} (s4)
			(s5) edge[loop above] node{$0$} (s5)
			(s3) edge[loop above] node{$0$} (s3)
			(s1) edge[above] node{$0$} (s2)
			(s1) edge[left] node{$0$} (s3)
			(s2) edge[right] node{$r>0$} (s4)
			(s2) edge[above] node{$0$} (s5);
			\end{tikzpicture}
			\caption{(Non-concavity example) A deterministic MDP
                          corresponding to Lemma
                          \ref{lemma:softmax-noncon} where  $
                          V^{\pi_\theta} (s)$ is not concave. 
                          Numbers on arrows represent the rewards for
                          each action.} 
%			\caption{A simple deterministic MDP corresponding to Lemma \ref{lemma:softmax-noncon} where  $ V^{\pi_\theta} (s)$ is non-concave. The numbers on arrows represent the rewards for each action.}
			\label{fig:noncon}
...
%                        \caption{A deterministic, chain MDP of length $H+2$. We consider a policy where $\pi(a | s_i) =\theta_{s_i,a}$ for $i=1,2,\ldots,H$. Rewards are $0$ everywhere other than $r(s_{H+1},a_1) = 1$.}
			\label{fig:chain}
\fi
	\end{figure}
