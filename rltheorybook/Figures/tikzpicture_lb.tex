...
\begin{tikzpicture}[
  ...
  obsbox/.style={draw,dashed,rounded corners,inner sep=2.8mm},
  % If you want arrowheads, change to:
  % aone/.style={thick,->},
...
% -------------------------
\node[align=left] at (3.6,2.45) {$\phi(s_h^c,a_1)=e_c$\\
$\phi(s_h^c,a_2)=e_{m+c}$\\
$\phi(g_h,a)=\frac{1}{\sqrt m}\sum_{c=1}^m e_c$};

% Legend box (slightly higher)
...
% -------------------------
\tikzset{obsbox/.style={draw,densely dotted,rounded corners,inner sep=2.8mm}}
...
% Labels: slightly DOWN and RIGHT (inside the box; avoids hitting the top border)
\def\lblx{12mm}
\def\lbly{-1.4mm}
\node[anchor=north west] at ([xshift=\lblx,yshift=\lbly]box0.north west) {$D_0$};
\node[anchor=north west] at ([xshift=\lblx,yshift=\lbly]box1.north west) {$D_1$};
\node[anchor=north west] at ([xshift=\lblx,yshift=\lbly]boxh.north west) {$D_h$};
\node[anchor=north west] at ([xshift=\lblx,yshift=\lbly]boxHm.north west) {$D_{H-2}$};
\node[anchor=north west] at ([xshift=\lblx,yshift=\lbly]boxH.north west) {$D_{H-1}$};
