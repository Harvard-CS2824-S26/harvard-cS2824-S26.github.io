...
infinite), where for each $f \in \H$ we have an associated policy
$\pi_f:\Scal \rightarrow \Acal$, which, for simplicity, we assume is deterministic.
Here, we could have that:
\begin{itemize}
\item $\H$ itself is a class of policies.
\item $\H$ is a set of state-action values, where for $f \in \Hcal$,
  we associate it with the greedy policy $\pi_f(s,h) = \argmax_a
  f_h(s,a)$, where $s\in\Scal$ and $h\in[H]$.
\item $\H$ could be a class of models (i.e. each $f \in \Hcal$ is an
...
\Xset$ and $y_i \in \{0,1\}$. Suppose we have a (finite or infinte) set $\Hcal$ of
binary classifiers where each $h \in \Hcal$ is a mapping of the form
$h :\Xset \to \{0,1\}$. Let $\ind(h(x) \ne y)$ be an indicator which
