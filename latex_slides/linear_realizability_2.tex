...
\begin{frame}{Roadmap: Proving LSVI Works}
    ...
    \begin{enumerate}
        \item \textbf{Regression Analysis:} Bound the error
          $\|\widehat{\theta}_h - \tilde\theta_h\|$ using OLS (where
          $\tilde\theta_h$ is the target.)
...
\begin{frame}{Step 1: The Regression at Stage $h$}
    ...
    \[
    \E[y_i | s_i, a_i] = (\Tcal_h \widehat{Q}_{h+1})(s_i, a_i) = (\widetilde\theta_h)^\top \phi(s_i, a_i)
    \]
    Thus, $y_i = (\widetilde\theta_h)^\top \phi(s_i, a_i) + \xi_i$, where $\xi_i$ is sub-Gaussian noise.
\end{frame}
...
\begin{frame}{Step 1: The Fixed Design Bound}
    ...
    \[
    \|\widehat{\theta}_h - \widetilde\theta_h\|_{\Lambda_h} \;\le\; c \cdot H \sqrt{\frac{d \log(H/\delta)}{N}}
    \]
...
\begin{frame}{Step 2: From Parameters to Pointwise Error}
    ...
    \[
    | (\widehat{\theta}_h - \widetilde\theta_h)^\top \phi(s,a) | 
    \;\le\; \|\widehat{\theta}_h - \widetilde\theta_h\|_{\Lambda_h} \sqrt{\phi(s,a)^\top \Lambda_h^{-1} \phi(s,a)}
    \]
