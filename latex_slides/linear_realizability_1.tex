...
% =============================================================================
% Part V: The Danger of Realizability (Analysis Breakdown)
% =============================================================================

\section{Where Analysis Fails Without Completeness}

\begin{frame}{The Critical Decomposition}
    Recall our backward induction analysis. The total error at stage $h$ is:
    \[
    \|\widehat{Q}_h - Q^\star_h\|_\infty \le \underbrace{\|\widehat{Q}_h - \Tcal_h \widehat{Q}_{h+1}\|_\infty}_{\text{Bellman Residual}} + \underbrace{\|\Tcal_h \widehat{Q}_{h+1} - \Tcal_h Q^\star_{h+1}\|_\infty}_{\text{Recursive Error}}
    \]
    
    The Recursive Error is bounded by the previous step's error. The analysis boils down to controlling the **Bellman Residual**.
    
    \vspace{0.3cm}
    \textbf{Decomposing the Residual:}
    Let the regression target be $y = \Tcal_h \widehat{Q}_{h+1}$. Our estimator $\widehat{Q}_h$ is the best linear fit to $y$.
    \[
    \text{Residual} \;\le\; \underbrace{\|\widehat{Q}_h - \Pi_\Fcal y\|_\infty}_{\text{Estimation Error (Variance)}} \;+\; \underbrace{\|\Pi_\Fcal y - y\|_\infty}_{\text{Approximation Error (Bias)}}
    \]
    \small
    \emph{(Here $\Pi_\Fcal$ is the projection onto linear functions under our data distribution).}
\end{frame}

\begin{frame}{The Off-Manifold Problem}
    \[
    \text{Residual} \;\le\; \text{Estimation} \;+\; \text{Approximation}
    \]

    \textbf{Case 1: Bellman Completeness}
    \begin{itemize}
        \item By assumption, $y = \Tcal_h \widehat{Q}_{h+1} \in \Fcal$.
        \item Therefore, $\Pi_\Fcal y = y$.
        \item \textbf{Approximation Error = 0.} We only fight variance (solved by $N$).
    \end{itemize}

    \vspace{0.3cm}

    \textbf{Case 2: Realizability Only (No Completeness)}
    \begin{itemize}
        \item Even if $Q^\star \in \Fcal$, the intermediate target $y = \Tcal_h \widehat{Q}_{h+1}$ may leave $\Fcal$.
        \item \textbf{Approximation Error $> 0$.}
        \item This is the "Off-Manifold" error. It exists even with infinite data.
    \end{itemize}
\end{frame}

\begin{frame}{Why Approximation is Deadly ($\sqrt{d}$ Amplification)}
    Why can't we just accept a small approximation error?
    
    \textbf{The Geometry of Linear Regression:}
    We control errors in the data norm ($L_2$), but we need uniform bounds ($L_\infty$).
    \[
    \|f\|_\infty \le \sqrt{d} \cdot \|f\|_{L_2(\rho)} \quad \text{(for D-optimal } \rho\text{)}
    \]
    
    \textbf{The Amplification Loop:}
    \begin{enumerate}
        \item At step $h+1$, we have error $\epsilon_{h+1}$.
        \item The target $\Tcal_h \widehat{Q}_{h+1}$ leaves the manifold.
        \item We project back to $\Fcal$. In the worst case, this projection aligns with the "spiky" directions of the basis.
        \item We pay the geometry factor: $\epsilon_h \approx \sqrt{d} \cdot \epsilon_{h+1}$.
    \end{enumerate}
    
    \vspace{0.2cm}
    \textbf{Result:} Over $H$ steps, error grows as $(\sqrt{d})^H$.
    \small \emph{(Lecture 3 will show this lower bound is tight!)}
\end{frame}
