...
\begin{frame}{Agenda}
    ...
    \textbf{Announcements}
    \begin{itemize}
    \item more project dates coming soon
    \item reading3 posted
    \end{itemize}
...
\begin{frame}{Task 2: Policy Evaluation}
   ...
   \textbf{Weaker Assumption:}
    \begin{itemize}
        ...
        \textbf{Result:}
    Basically the same as LSVI (with slighly better $H$ dependence).
\end{frame}
...
\begin{frame}[plain]
  \vfill
  \begin{center}
    {\Huge \bfseries Offline Lower Bounds with \alert{Realizatiblity}+Coverage}
  \end{center}
  \vfill
\end{frame}
\addtocounter{framenumber}{-1}
...
\pause        \item \textbf{Assumption 1 (All-Policies Realizability):} 
        For \emph{every} policy $\pi$, $Q^\pi_h$ is linear in $\phi$.
\pause        \item \textbf{Assumption 2 (Perfect Coverage):} 
    ...
    \vspace{0.3cm}
\pause    \textbf{The Question:} Under these ideal conditions (Realizability + Perfect Coverage), can we learn $V^\pi$ with $\poly(d, H)$ samples?
\end{frame}
...
\pause    \textbf{1. The Observed States are Tabular:}
...
\pause    \textbf{2. The Aggregator is Constrained:}
...
\pause    \textbf{1. Tabular Learning on Observed States (Base Case)}
...
\pause    \textbf{2. The Recursive Update.}
...
\pause      \item \textbf{The Question:} Is there a hard instance --- where
...
\pause    \textbf{The Assumption (Weakening):}
    ...
    \vspace{0.3cm}
\pause    \begin{block}{Theorem (Hardness of Linear $Q^\star$)}
...
\pause        \item \textbf{Linear $Q^\star$ Realizability:}
        \begin{itemize}
...
\pause        \item \textbf{All-Policy Realizability:}
        \begin{itemize}
